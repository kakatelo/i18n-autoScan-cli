...
program
  ...
  .option('--zh-reg <item>', '中文正则')
  .option('--ignore-key-rules <items>', '忽略被提取的内容的规则', commaSeparatedList)
  .option('--custom-ignore-wrap <items>', '自定义忽略被包裹的内容，需要包含三个group，前中后', commaSeparatedJson)
  .option('--ignore-wrap <items>', '忽略被包裹，需要包含三个group，前中后', commaSeparatedJson)
  .option('--ignore-disable-wrap <items>', '忽略禁用包裹，需要包含三个group，前中后', commaSeparatedJson)
...
const config = {
  ...
  customIgnoreWrap: [{ name: 'i18n', regex: /(t\s*\()([\s\S]*?)(\))/g }],
    ...
    .on('end', async () => {
      ...
      if (filesPathArr.length) {
        ...
        if (config.outCsv) {
          ...
          await csv.toDisk(path.resolve(outPath, 'auto-i18n-extract-output.csv'), { bom: true }) // bom为true 解决中文乱码问题
        }
        console.log('---提取完成---')
      } else {
