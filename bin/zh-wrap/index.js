...
program
  ...
  .option('--ignore-pre-geg <items>', '被忽略的前缀，是个数组', commaSeparatedList)
  .option('--ignore-text <items>', '被忽略的文本，是个数组', commaSeparatedList)
  .option('--ignore-attr <items>', '被忽略的属性，该属性的value值如果是中文，则不会被包裹，是个数组', commaSeparatedList)
  .option('--disabled-rules <items>', '如果满足匹配的内容，就忽略包裹', commaSeparatedList)
...
const config = {
  ...
  ignoreI18nFileRules: [],
  // 被忽略的前缀
  ignorePreReg: [/T\s*\([\s\n]*$/, /Tl\s*\([\s\n]*$/, /console\.(?:log|error|warn|info|debug)\s*\(\s*$/, new RegExp('//.*')],
  // 被忽略的文本
  ignoreText: [/T\(/, /Tl\(/],
  ...
  newFileContent = newFileContent.replace(templateReg, function (match, templateKey, index) {
    let newTemplateKey = templateKey.replace(i18nStrRegForBacktick, function (match, key, index) {
      ...
      if (variables.length) {
        ...
        // 转换后的结果 $t("My name is {0} and I am {1} years old.", [name, age])
...
function processHtmlFile(fileContent) {
  ...
  const styleCodes = []
  const linkCodes = []
  newFileContent = newFileContent.replace(/(<script[\s\S]*?<\/script>)/gi, function (match, key, index) {
    const count = scriptCodes.length
    scriptCodes.push(match)
    return match.replace(key, `@@scriptCodes_${count}@@`)
  })
  newFileContent = newFileContent.replace(/(<style[\s\S]*?<\/style>)/gi, function (match, key, index) {
    const count = styleCodes.length
    styleCodes.push(match)
    return match.replace(key, `@@styleCodes_${count}@@`)
  })
  newFileContent = newFileContent.replace(/(<link[\s\S]*?<\/link>)/gi, function (match, key, index) {
  ...
  newFileContent = newFileContent.replace(/(@@scriptCodes_.*?@@)/gi, function (match, key, index) {
    const i = match.match(/@@scriptCodes_(.*?)@@/)[1]
    return match.replace(key, scriptCodes[i].replace(/\$/g, '$$$$'))
  })
  newFileContent = newFileContent.replace(/(@@styleCodes_.*?@@)/gi, function (match, key, index) {
    const i = match.match(/@@styleCodes_(.*?)@@/)[1]
    return match.replace(key, styleCodes[i].replace(/\$/g, '$$$$'))
  })
  newFileContent = newFileContent.replace(/(@@linkCodes_.*?@@)/gi, function (match, key, index) {
